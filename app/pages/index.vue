  ...
  <div class="space-y-12">
    <section class="grid min-h-80 items-center gap-10 md:grid-cols-[1fr_18rem]">
      <div>
        ...
        <h1 class="mt-5 font-display text-5xl leading-[0.98] tracking-[-0.055em] sm:text-6xl lg:text-7xl">
          今天，先让<br><em class="text-primary">耳朵认出</em>英语。
        </h1>
        <p class="mt-6 max-w-md leading-7 text-muted">
      ...
      <UCard class="hidden md:block" :ui="{ body: 'space-y-5 p-7' }">
        <div class="flex items-end justify-between">
          <div>
            ...
            <p class="mt-2 font-display text-6xl">{{ completion }}<small class="text-xl text-primary">%</small></p>
          </div>
    ...
    <section>
      <div class="mb-5 flex items-end justify-between gap-4">
        <div>
          ...
          <h2 class="mt-2 font-display text-3xl tracking-tight">今天的三段路</h2>
        </div>
      ...
      <div class="grid gap-3 md:grid-cols-3">
        <UCard v-for="step in steps" :key="step.number" :ui="{ body: 'flex min-h-52 flex-col p-6' }">
          ...
          <p class="mt-2 flex-1 leading-6 text-muted">{{ step.description }}</p>
    ...
    <section class="grid gap-3 md:grid-cols-[1.1fr_0.9fr]">
      ...
      <UCard class="bg-primary/10" :ui="{ body: 'p-7' }">
        ...
        <div class="mt-5 grid grid-cols-[1fr_auto_1fr] items-center gap-4">
          <div><strong class="font-display text-4xl">{{ averageFirstListen || '—' }}<small class="text-base text-primary">%</small></strong><span class="block text-xs text-muted">第一次裸听</span></div>
          <UIcon name="i-lucide-arrow-right" class="text-primary" />
          <div><strong class="font-display text-4xl">{{ averageReplay || '—' }}<small class="text-base text-primary">%</small></strong><span class="block text-xs text-muted">再次裸听</span></div>
        </div>
